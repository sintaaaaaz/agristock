                ...
                <form action="{{ route('outgoing-goods.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="transaction_code"
                            ...
                            placeholder="Contoh: BK-001" value="{{ old('transaction_code') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="purpose" class="form-control @error('purpose') is-invalid @enderror"
                            placeholder="Contoh: Pasar Induk / Toko A" value="{{ old('purpose') }}">
                        @error('purpose')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select name="product_id" class="form-select @error('product_id') is-invalid @enderror">
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}"
                                    ...
                                    {{ $product->product_code }} - {{ $product->product_name }} (Tersedia:
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="quantity" class="form-control @error('quantity') is-invalid @enderror"
                            min="1" placeholder="0" value="{{ old('quantity') }}">
                        @error('quantity')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
