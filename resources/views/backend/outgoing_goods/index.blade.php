                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('outgoing-goods.show', $item->id) }}"
                                                class="btn btn-info btn-sm text-white px-2">
                                                <i class="fa-solid fa-eye me-1"></i> Detail
                                            </a>
                                            <form action="{{ route('outgoing-goods.destroy', $item->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger btn-sm px-2"
                                                    onclick="return confirm('Hapus transaksi ini? Stok gudang akan otomatis dikembalikan.')">
