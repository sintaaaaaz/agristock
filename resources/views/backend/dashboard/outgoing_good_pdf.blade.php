...
<head>
    ...
    <style>
        ...
        th { background-color: #dc3545; color: white; } /* Warna Merah untuk Barang Keluar */
                ...
                <tr>
                    ...
                    <td>
                        @foreach($item->outgoingGoodDetails as $detail)
                            {{ $detail->product->product_name ?? 'Produk Terhapus' }} ({{ $detail->quantity }} {{ $detail->product->unit->name ?? '' }}), 
                        @endforeach
                    </td>
                </tr>
            @endforeach
