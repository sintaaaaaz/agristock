                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            @forelse($incomingGoods as $key => $item)
                                <tr style="height: 60px;">
                                    ...
                                    <td class="text-center"><span class="badge bg-success-subtle text-success border rounded-pill px-3">Berhasil</span></td>
                                    <td>
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('incoming-goods.show', $item->id) }}" class="btn btn-info text-white px-3 py-2" style="font-size: 0.9rem;">
                                                <i class="fa-solid fa-eye me-1"></i> Detail
                                            </a>
                                            <form action="{{ route('incoming-goods.destroy', $item->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger px-3 py-2" style="font-size: 0.9rem;" onclick="return confirm('Yakin ingin menghapus data ini?')">
...
<script>
    @if(Auth::user()->role_id == 1)
        new Chart(document.getElementById('chartBarangMasuk'), { type: 'bar', data: { labels: {!! json_encode($labels) !!}, datasets: [{ label: 'Qty Masuk', data: {!! json_encode($totalsIn) !!}, backgroundColor: '#198754' }] }, options: { responsive: true, maintainAspectRatio: false } });
        new Chart(document.getElementById('chartBarangKeluar'), { type: 'bar', data: { labels: {!! json_encode($labels) !!}, datasets: [{ label: 'Qty Keluar', data: {!! json_encode($totalsOut) !!}, backgroundColor: '#dc3545' }] }, options: { responsive: true, maintainAspectRatio: false } });
    @endif
</script>
@endsection